...
pack.addFormula({
  ...
  execute: async function ([rrule, after, before, limit, include = false], context) {
    limit = Math.max(limit, MAX_CONSUMED_RECURRENCES); // silently cap
    const rule = RRule.fromString(rrule);
    return rule.between(after, before, include, limitingIterator(limit)).map(Date.prototype.toISOString);
  }
});
...
pack.addFormula({
  ...
  execute: async function([rrule, date, include = false], context) {
    const rule = RRule.fromString(rrule);
    return rule.before(date, include).toISOString();
  }
});
